...
import com.example.shittleproject2.PillowNFCWriteException.NFCErrorType;
...
public class PillowNfcManager {
	...
	public void onActivityNewIntent(Intent intent) {
		...
		if (writeText == null)
			...
			} catch (PillowNFCWriteException exception) {
	 ...
	 * @param tag
	 * @param data
	 * @throws PillowNFCWriteException
	 */
	protected void writeTag(Context context, Tag tag, String data) throws PillowNFCWriteException {
		...
		if (ndef != null) {
			...
			try {
				...
				throw new PillowNFCWriteException(PillowNFCWriteException.NFCErrorType.unknownError);
			}
			// Make sure the tag is writable
			if (!ndef.isWritable()) {
				throw new PillowNFCWriteException(NFCErrorType.ReadOnly);
			}

			// Check if there's enough space on the tag for the message
			int size = message.toByteArray().length;
			if (ndef.getMaxSize() < size) {
				throw new PillowNFCWriteException(NFCErrorType.NoEnoughSpace);
			}
			...
			try {
				...
				throw new PillowNFCWriteException(PillowNFCWriteException.NFCErrorType.tagLost, tle);
			} catch (IOException ioe) {
				throw new PillowNFCWriteException(NFCErrorType.formattingError, ioe);// nfcFormattingErrorTitle
			} catch (FormatException fe) {
				throw new PillowNFCWriteException(NFCErrorType.formattingError, fe);
			}
		} else {
			...
			if (format != null) {
				try {
					...
					throw new PillowNFCWriteException(NFCErrorType.tagLost, tle);
				} catch (IOException ioe) {
					throw new PillowNFCWriteException(NFCErrorType.formattingError, ioe);
				} catch (FormatException fe) {
					throw new PillowNFCWriteException(NFCErrorType.formattingError, fe);
				}
			} else {
				throw new PillowNFCWriteException(NFCErrorType.noNdefError);
			}
		}
